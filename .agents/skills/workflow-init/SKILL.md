---
name: workflow-init
description: Install and configure Vercel Workflow SDK before it exists in node_modules. Use when the user asks to "install workflow", "set up workflow", "add durable workflows", "configure workflow sdk", or "init workflow" for Next.js, TanStack Start, Express, Hono, Fastify, NestJS, Nitro, Nuxt, Astro, SvelteKit, or Vite.
metadata:
  author: Vercel Inc.
  version: '1.4'
---
...
2. **TanStack Start** - `@tanstack/react-start` or `@tanstack/solid-start` dep, or `src/start.ts`
3. **Nuxt** - `nuxt` dep or `nuxt.config.*`
4. **SvelteKit** - `@sveltejs/kit` dep or `svelte.config.*`
5. **Astro** - `astro` dep or `astro.config.*`
6. **NestJS** - `@nestjs/core` dep or `nest-cli.json`
7. **Nitro** - `nitro` dep or `nitro.config.*`
8. **Express** - `express` dep
9. **Fastify** - `fastify` dep
10. **Hono** - `hono` dep
11. **Vite** - `vite` dep (and not matched above)
...
- **Web app**: Next.js / TanStack Start / Nuxt / SvelteKit / Astro
