...
`routeTree.gen.ts` is generated by the TanStack Router Vite plugin. When the dev server is already running (`pnpm --filter web dev`), adding or renaming files under `apps/web/src/routes/` updates the route tree automatically — do not run a separate generate step in that case.
